    ...
    try {
      ...
      const bidders = await bidderService.biddersForClient(
        {client_entity_id: body.client_entity_id,
          auction_entity_id: body.auction_entity_id}
        );
        let personalizations:any = await Promise.all(
          bidders.map(async (bidder) => {
            ...
            return {
              to: [{ email: "testjst@yopmail.com" || bidder.email }],
        ...
        // personalizations.email_reply_to=body?.email_reply_to;
        // personalizations.email_subject=body?.email_subject;
        // personalizations.from_name=body?.email_from_name;
        // personalizations.email_from=body?.email_from;
        // personalizations.email_from=body?.email_from;
        //  await Promise.all([
        let options: any = {
          email_reply_to: body?.email_reply_to,
