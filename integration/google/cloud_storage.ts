...
export class CloudStorage {
  ...
  public async uploadFile(fileToUpload: INewUploadFile): Promise<INewUploadFile> {
    try {
      let resizedBuffer = fileToUpload.file.data;

      const fileNameNoExtension = path.parse(fileToUpload.file.name).name;
      const fileExtension: any = path.parse(fileToUpload.file.name).ext.toLowerCase();
      ...
      if (SUPPORTED_IMAGE_EXTENSIONS.includes(`${fileExtension}`)) {
        ...
        console.log("this was either not an image, or the image is not supported. conversion skipped!");
      }

      const sanitizedFileName = `${stripSpecialCharacters({ sourceString: fileNameNoExtension })}${fileExtension}`;
  ...
  public async getBucketMetadata() {
    const [metadata] = await this.storage.bucket(process.env.GOOGLE_BUCKET_NAME).getMetadata();

    console.info(`Bucket ${process.env.GOOGLE_BUCKET_NAME} was updated with a CORS config:`, metadata);
  }
  ...
  public async configureBucketCors() {
    ...
    await this.storage.bucket(process.env.GOOGLE_BUCKET_NAME).setCorsConfiguration(options);

    console.info(`Bucket ${process.env.GOOGLE_BUCKET_NAME} was updated with a CORS config:`, options);
  }
