...
import { AuctionTutorRepository } from "../../schemas/redis/auction_tuter";
      ...
      const remainingTimeFormatted = `${remainingTime
        ...
        .padStart(2, "0")}:${remainingTime
          ...
          **/
          ...
          async saveUrl(url: string, entity_id: any) {
            const client_entity_id = entity_id;
            
            const existing = await AuctionTutorRepository.search()
            .where("client_entity_id")
            .eq(client_entity_id)
            .return.first();
            ...
            if (existing) {
              existing.url = url;
              await AuctionTutorRepository.save(existing);
              return { updated: true, data: existing };
            } else {
              const newEntry = await AuctionTutorRepository.save({
                url,
                client_entity_id,
              });
          ...
          async getUrl(client_entity_id:string) {
            let auctuionUrl=  await AuctionTutorRepository.search().where("client_entity_id").eq(client_entity_id).return.all();
