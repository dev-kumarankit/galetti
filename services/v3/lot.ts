...
import { lot_start ,lot_countdown} from "../../integration/redis/bull/queues/lot_status_update";
...
export class LotService3 {
  ...
  public async createLot(lot: ILot) {
    ...
    if (diff <= 120000) {
      // Emit countdown immediately
      this.rtc_di.emitLotCountdown(entityId,{
        lot_entity_id: entityId,
        lot_number: obj.lot_number,
        auction_entity_id: obj.auction_entity_id,
        seconds_remaining: Math.floor(diff / 1000),
      });
    } else {
      // Schedule countdown Bull job
      lot_countdown.add(
        { lot_entity_id: entityId },
        { delay: diff - 120000 }
      );
    ...
    lot_start.add(
      ...
      {
        delay: moment(moment(obj.date_from)).diff(
  ...
  public async updateLot(entity_id: string, lot: ILot) {
   ...
   const diff = moment(objToSave.date_from).diff(moment().tz("Africa/Johannesburg"), "milliseconds");
      ...
      if (diff <= 120000) {
        ...
        this.rtc_di.emitLotCountdown(entityId,{
          lot_entity_id: entityId,
          lot_number: objToSave.lot_number,
          auction_entity_id: objToSave.auction_entity_id,
          seconds_remaining: Math.floor(diff / 1000),
        });
      } else {
        // Schedule countdown Bull job
  ...
  console.log("Lot updated with bull", entityId);
