...
//   if (!lot || !lot.auction_entity_id) throw new ValidationError("Lot not found!");

//   // Fetch active auto-bidders
...
//   if (activeAutoBidders.length === 0) return;
...
//     if (triggeringUserId && bidder.user_entity_id === triggeringUserId) return false;
//     return bidder.max_amount > currentHighestAmount;
//   });

//   if (eligibleBidders.length === 0) return;

//   // Pick earliest eligible bidder
//   const nextBidder = eligibleBidders[0];

//   // Determine next bid amount
//   let nextBidAmount = currentHighestAmount;
//   if (!highestBid) {
//     nextBidAmount = lot.starting_price;
//   } else {
...
//   // Place bid as auto-bid
//   // await placeBid(autoBidObj, true);
//   const response = await bidService.placeBid(autoBidObj);

//   // If multiple bidders remain, trigger next round
//   if (eligibleBidders.length > 1) {
//     setTimeout(() => {
//       this.triggerAutoBidForLot.call(this, lotId, nextBidder.user_entity_id);
//     }, 500);
//   }
// }
...
//   // Filter eligible bidders
//   const eligibleBidders = activeAutoBidders.filter(bidder => {
//     // Skip if they are the one who triggered this round
//     if (triggeringUserId && bidder.user_entity_id === triggeringUserId) 
//       {
//       console.log("returning ..... they are the one who triggered this round")
//       return false;
      
//     }
//     // Skip if their max amount isn't high enough
//     if (bidder.max_amount <= currentHighestAmount) {
//       console.log("returning .....  max amount isn't high enough")
      
//       return false;}
//       // Skip if they already hold the highest bid
//       if (highestBid && highestBid.user_entity_id === bidder.user_entity_id) {
        
//         console.log("returning .....  they already hold the highest bid");
...
//         console.log("eligibleBidders.length === 0");
//         return;
//       }
...
//       // Trigger next round if multiple bidders remain
//       if (eligibleBidders.length > 1) {
//         setTimeout(() => {
//           this.triggerAutoBidForLot.call(this, lotId, nextBidder.user_entity_id);
//         }, 500);
